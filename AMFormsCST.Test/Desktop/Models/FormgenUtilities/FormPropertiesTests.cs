...
public class FormPropertiesTests
{
    ...
    public void GetDisplayProperties_ReturnsExpectedProperties()
    {
        ...
        // Assert
        Assert.Contains(displayProps, dp => dp.Name == "Title" && Equals(dp.Value, dotFormgen.Title));
        Assert.Contains(displayProps, dp => dp.Name == "FormType" && Equals(dp.Value, dotFormgen.FormType));
        Assert.Contains(displayProps, dp => dp.Name == "Category" && Equals(dp.Value, dotFormgen.Category));
        Assert.Contains(displayProps, dp => dp.Name == "Username" && Equals(dp.Value, dotFormgen.Username));
        Assert.Contains(displayProps, dp => dp.Name == "BillingName" && Equals(dp.Value, dotFormgen.BillingName));
        Assert.Contains(displayProps, dp => dp.Name == "TradePrompt" && Equals(dp.Value, dotFormgen.TradePrompt));
        Assert.Contains(displayProps, dp => dp.Name == "SalesPersonPrompt" && Equals(dp.Value, dotFormgen.SalesPersonPrompt));
        Assert.Contains(displayProps, dp => dp.Name == "PublishedUUID" && Equals(dp.Value, dotFormgen.Settings.UUID));
        Assert.Contains(displayProps, dp => dp.Name == "TotalPages" && Equals(dp.Value, dotFormgen.Settings.TotalPages));
    }
}